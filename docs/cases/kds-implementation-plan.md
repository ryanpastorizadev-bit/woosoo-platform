...
tier: 3
next_agent: 'contrarian (P2 starts a fresh Tier 3 chain — Claude Code, not Cursor)'
branch: 'nexus dev (agent/kds-phase-0 merged); next: agent/kds-p2-recall'
interrupted: false
updated: 2026-06-09
...
- branch: nexus dev (agent/kds-phase-0 merged); next: agent/kds-p2-recall
- status: IN_PROGRESS
- last_completed_agent: specialist (P0 + P1 landed on nexus dev through 13afccf: read-only board, advance/toggle, migration, ItemToggled, transaction guards, 13 tests green)
- next_agent: contrarian (P2 starts a fresh Tier 3 chain — Claude Code, not Cursor)
...
| **P2** recall + voided-from-feed | 🔴 BACKEND NOT DONE | enum `OrderStatus` has **no** recall edge (`IN_PROGRESS => [READY, VOIDED]` only); **no** `KdsController::recall`; **no** `/kds/.../recall` route; `order-state.contract.md` has no recall language. Frontend has recall scaffolding only. This is the B5.1 STOP-point. |
