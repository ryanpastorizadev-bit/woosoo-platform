...
`woosoo-nexus`, `tablet-ordering-pwa`, and `woosoo-print-bridge`. For any
...
| Area | Path | Role |
| --- | --- | --- |
| Platform governance | `.` | Agent protocol, contracts, case files, orchestration state, deployment docs, and platform-root Docker/deployment guidance. |
| Nexus | `woosoo-nexus/` | Laravel backend and admin UI. Owns API contracts, POS/Krypton integration, order truth, Reverb broadcasts, print-event orchestration, and backend validation. |
| Tablet PWA | `tablet-ordering-pwa/` | Nuxt 3 customer tablet app. Owns kiosk interaction, package/menu browsing, local UI/session recovery, and Echo/Reverb client behavior. |
| Print Bridge | `woosoo-print-bridge/` | Flutter Android print relay. Owns printer identity, heartbeat, queue intake, Bluetooth dispatch, and reserve/ack/failed handling. |

The apps form a chain. The tablet captures customer intent, Nexus validates and
persists business truth, and the Print Bridge proves the last-mile printer
outcome.
...
- Print delivery must be idempotent at the reserve/ack/failed lifecycle.
