...
|---|---|---|---|---|---|---|---|
| P1 | NEX-CASE-011 | woosoo-nexus / POS | Code merged to dev (PR #163). **Ops step remaining (Pi):** disable 3rd-party Krypton printer in POS config so only the BT thermal printer fires. Case: `nex-case-011-duplicate-order-printing.md` | 3 | none | code-merged; POS config pending (Pi ops) | #140 |
| P1 | NEX-CASE-014 | woosoo-nexus | SESSION_DOMAIN fix + WOOSOO_ENV profile switch — APPROVED 2026-06-05, merged to dev. Operator: clear pinned SESSION_DOMAIN from live .env; set WOOSOO_ENV; re-run apply script. Case: `nex-case-014-session-domain-login-419.md` | 2 | none | done (code); Pi .env correction pending | — |
