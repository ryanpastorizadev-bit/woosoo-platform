...
| P2 | INFRA-CASE-004 | woosoo-platform | **Re-scoped → one robust deploy command + secure env.** `deploy.sh`/`deploy-all.sh` hardened: retry on git fetch/composer/build, composer hydration (bind-mount vendor gap), tablet UI cache-bust, unbypassable doctor gate; `.env`+backups `chmod 600`; docs collapsed to one config truth (`./woosoo.env` via `init-woosoo-env.sh`); check.sh dedup. Executioner APPROVED (local gates). Case: `infra-case-004-script-flow-unification.md` | 2 | none | PR #45 open → dev (APPROVED locally); Pi runtime = Bucket B | #45 |
...
| Priority | Case ID | App | Description | Tier | Dep | Status | GH |
|---|---|---|---|---|---|---|---|
| P2 | TAB-CASE-011 | tablet-ordering-pwa | Active-order recovery filter — **COMPLETE 2026-06-07** (APPROVED; PR #199 merge pending). Case: `tab-case-011-active-order-recovery-filter.md` | 2 | none | complete | — |
