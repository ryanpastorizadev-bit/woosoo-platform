...
| P2 | INFRA-CASE-004 | woosoo-platform | **Re-scoped → one robust deploy command + secure env.** `deploy.sh`/`deploy-all.sh` hardened: retry on git fetch/composer/build, composer hydration (bind-mount vendor gap), tablet UI cache-bust, unbypassable doctor gate; `.env`+backups `chmod 600`; docs collapsed to one config truth (`./woosoo.env` via `init-woosoo-env.sh`); check.sh dedup. Executioner APPROVED (local gates). Case: `infra-case-004-script-flow-unification.md` | 2 | none | APPROVED on branch `agent/infra-004-deploy-hardening`; merge to dev pending; Pi runtime = Bucket B | — |
